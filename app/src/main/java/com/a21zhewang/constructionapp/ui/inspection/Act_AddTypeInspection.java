...
public class Act_AddTypeInspection extends BaseActivity implements RadioGroup.OnCheckedChangeListener{
    ...
    @BindView(R.id.et_cast_number)
    EditText et_cast_number;
    ...
    public int getLayoutId() {
        return R.layout.act_addtype_inspection;
    }
    ...
    public void initViews() {
        ...
        et_cast_number.setInputType(InputType.TYPE_CLASS_NUMBER | InputType.TYPE_NUMBER_FLAG_DECIMAL);
...
//        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, R.layout.item_spinner_defalut,spinnerlist);
        adapter = new ArrayAdapter<String>(Act_AddTypeInspection.this,R.layout.item_spinner_defalut,spinnerlist);
    ...
    public void getSpinner(){
        ...
        if("com.a21zhewang.constructionapp3".equals(AppUtils.getPackageName(getApplicationContext()))
            ...
            XUtil.postjsondata(object, "GetReasonList", callback);
        }else {
            XUtil.postjsondatasj(object, "GetReasonList", callback);
        }
    }
    @Override
...
//                            remove(getData().size()-1);
    ...
    public void initAlert(){
        new ActionSheetDialog(Act_AddTypeInspection.this)
                .builder()
                .setCancelable(false)
    ...
    public void PictureSelector(){
        PictureSelector.create(Act_AddTypeInspection.this).openGallery(PictureMimeType.ofImage())
                .maxSelectNum(9)
                .imageSpanCount(4)
    ...
    public void PictureCarmerSelector(){
        PictureSelector.create(Act_AddTypeInspection.this).openCamera(PictureMimeType.ofImage())
                .maxSelectNum(9)
                .imageSpanCount(4)
                .selectionMode(PictureConfig.MULTIPLE)
                .previewImage(true)
...
//                            remove(getData().size()-1);
    ...
    public void initData() {
        ...
        back.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Act_AddTypeInspection.this.finish();
            }
        });
    }
    ...
    public void submitDate(int status){
        ...
        if(status == 1){
            postJsonAndFile(object.toString(),files2,"SaveBroadCastItemResult",status);
        }
        if(status == 2){
            postJsonAndFile(object.toString(),files3,"SaveBroadCastItemResult",status);
        }
    }
    ...
    private void postjson(JSONObject object, final int status){
        ...
        if("com.a21zhewang.constructionapp3".equals(AppUtils.getPackageName(getApplicationContext()))
                || "com.a21zhewang.constructionapp.cqjg".equals(AppUtils.getPackageName(getApplicationContext()))){
            XUtil.postjsondata(object, "SaveBroadCastItemResult", callback);
        }else {
            XUtil.postjsondatasj(object, "SaveBroadCastItemResult", callback);
        }
    }
    /**
     * 安全|有隐患 JSON+FILE提交
    ...
    private void postJsonAndFile(String json, List<String> files, String method, final int status) {
        ...
        if("com.a21zhewang.constructionapp3".equals(AppUtils.getPackageName(getApplicationContext()))
                || "com.a21zhewang.constructionapp.cqjg".equals(AppUtils.getPackageName(getApplicationContext()))){
            XUtil.updatas(JsonUtils.getjsonobj(json), files, method, callback);
        }else {
            XUtil.updatassj(JsonUtils.getjsonobj(json), files, method, callback);
        }
    }
    /**
     * 上行参数
    ...
    private JSONObject getjsonArray(int status) {
        try {
            ...
            commitObject.put("recordRLID",recordRLID);
                    ...
                    if(path != null){
                        if(selectTag == 1){
                            if(TextUtils.isEmpty(xPoint) && TextUtils.isEmpty(yPoint)){
                                Bitmap bitmap = SendImageUtil.createWatermark(Act_AddTypeInspection.this,
            ...
            if (status == 2) {
                commitObject.put("deductGrade",et_cast_number.getText().toString());
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == Activity.RESULT_OK && requestCode == 133) { //安全图库
            selectTag = 1;
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == 135) { //有隐患图库
            selectTag = 1;
    ...
    public void getLocation(){
        Location location = LocationUtils.getInstance(Act_AddTypeInspection.this ).showLocation();
        if (location != null) {
