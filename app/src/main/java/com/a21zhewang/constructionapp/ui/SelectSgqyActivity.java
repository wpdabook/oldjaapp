...
public class SelectSgqyActivity  extends BaseActivity{
    ...
    private BaseQuickAdapter<ProjectArea, BaseViewHolder> baseQuickAdapter;
    private final List<String> strs=new ArrayList<>();//文字拼接
    private final List<String> ids=new ArrayList<>();//文字拼接
    private final List<List<ProjectArea>> pjlists=new ArrayList<>();//记录每个层级的集合
    private final List<ProjectArea> pjs=new ArrayList<>();//记录点击的值
    public final static int RESQUST_CODE_SELECTSGYQ=333;
    public final static int RESQUST_CODE_SELECT_STAGE=336;
    ...
    public void initViews() {
        ...
        baseQuickAdapter = new BaseQuickAdapter<ProjectArea, BaseViewHolder>
                (R.layout.checklist_childitem, null) {


            @Override
            protected void convert(BaseViewHolder helper, final ProjectArea item) {
                helper.setText(R.id.checklist_childitem_tv,item.getProjectAreaName());
                helper.itemView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        List<ProjectArea> list = item.getProjectAreaList();
                        strs.add(item.getProjectAreaName());
                        ids.add(item.getProjectAreaID());
                        pjlists.add(list);
                        pjs.add(item);
                        if (list !=null&& list.size()>0){
                          ...
                          //  data.putParcelableArrayListExtra("list", (ArrayList<? extends Parcelable>) list2Tree(pjs));
                            data.putExtra("projectArea", item);
                            if (pjs.size()>0)
                                data.putParcelableArrayListExtra("etpInfoList", (ArrayList<? extends Parcelable>) pjs.get(pjs.size()-1).getEtpInfoList());
    ...
    public void initData() {
        ArrayList<ProjectArea> parcelableArrayListExtra = getIntent().getParcelableArrayListExtra("list");
