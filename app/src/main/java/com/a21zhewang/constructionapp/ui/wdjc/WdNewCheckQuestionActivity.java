...
public class WdNewCheckQuestionActivity extends BaseActivity implements WdOnFragmentInteractionListener {
...
//        int checkType = getIntent().getIntExtra("checkType",-1);
        ...
        if (isSafe==0){
            ...
            mDataList.add("安全");
            fragments.add(WdADDAQDKFragment.newInstance("",""));
            magicIndicator.setVisibility(View.GONE);
        }else if (isSafe==1){
            mDataList.add("隐患");
            magicIndicator.setVisibility(View.GONE);
            fragments.add(WdADDYHWTFragment.newInstance("",""));
        }else{
            mDataList.add("安全");
            mDataList.add("隐患");
            fragments.add(WdADDAQDKFragment.newInstance("",""));
