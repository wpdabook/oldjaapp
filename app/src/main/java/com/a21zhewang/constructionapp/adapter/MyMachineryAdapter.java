...
public class MyMachineryAdapter extends BaseAdapter {

	private Context context = null;
	private List<MachineryData> datas = null;
	...
	public MyMachineryAdapter(Context context, List<MachineryData> datas) {
		this.datas = datas;
	...
	public View getView(final int position, View view, ViewGroup parent) {
		final MachineryData machineryData = datas.get(position);
		...
		tvTitle.setText(machineryData.getConfigName());
		...
		cbCheck.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView,
				...
				if(isChecked){
					...
					machineryData.setChecked(false);
				}
			}
		});
...
//		}
	 ...
	 * 增加一项的时候
	 */
	public void add(MachineryData machineryData) {
		this.datas.add(0, machineryData);
		// 让所有项目都为不选择
		initDate(false);
	}
